...
class GameViewModel(application: Application) : AndroidViewModel(application) {
    ...
    // Use a simple counter to trigger recomposition instead of cloning the entire board
    var boardVersion by mutableStateOf(0)
        private set

    val board: GameBoard
        get() {
            // Access boardVersion to create dependency
            boardVersion
            return engine.board
        }
        ...
        get() {
            ...
            for (y in 0 until gameConfig.rows) {
                for (x in 0 until gameConfig.cols) {
                    engine.board.getCell("${x}_${y}")?.let { list += it }
                }
            }
            return list
        }
